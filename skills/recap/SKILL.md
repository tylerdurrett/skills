...
- **Length depends on window.** `today`: 3 to 6 sentences in a single short paragraph. `week`: 6 to 12 sentences, may break into 2 paragraphs when there are clearly distinct themes. `upcoming`: 3 to 6 sentences. No headers (no `##`, `###`), no horizontal rules, no frontmatter in any window. Bold is permitted for feature or PRD names.
- **Plain English over git/GitHub jargon.** See "Voice and tone" below — this is the most important rule.
- **Tie every user-meaning claim to a merged PR.** For past windows, only describe features whose corresponding PR is present in `prsMerged`. Do not bridge from a PRD's `## User Stories` to "we shipped X" without a merged PR backing it. PRD user stories capture original intent; some stories get cut (the helper filters `wontfix` closures out of `issuesClosed`, but does not filter the source PRD's user stories), some get deferred, and the recap is about what landed, not what the PRD originally promised.
...
- For internal tooling and workflow change, translate mechanism into the shift in how the team operates or what it unblocks. Reserve this for work that meaningfully changes the day, not minor skill churn.
  ...
  - **Good:** "We laid groundwork for thinking about the project at a higher level than individual PRDs, with a north-star doc, a roadmap, and the concept of epics that group related PRDs."
...
- Phrases like "merged to main", "rebased", "fast-forward", "integration branch", "PRD branch", "sub-issue rollup", "ahead/behind".
...
The window's lead phrasing has to match what actually happened. Pick one of four tiers, evaluated top-down — the first match wins. For `week`, evaluate the magnitude across the whole 7-day window, not the most recent day; a week is "headline" if a PRD or epic landed at any point in it.

- **Headline.** A `type:prd` or `type:epic` issue closed with its PR landing on `main` in the window. The recap leads strongly: "Today we shipped **the [PRD title]** — [one-sentence user-facing summary]." Reserve headline phrasing for this case only. PRDs and epics that did not land on `main` (still open, or merged to a parent integration branch but not promoted) are not headlines.

- **Notable.** No PRD/epic ship, but a `size:multi-pr` slice closed in the window, or 3+ individual closures landed, or a single closure is independently noteworthy (a long-standing bug fix, a user-visible polish that meaningfully improves a flow). Lead with a soft anchor: "The most useful change today...", "Today's biggest move...", "Today we polished...".
...
**The closer must be sourced from `inProgressSlices` in the helper output**, not extrapolated from a closed PRD's user stories. `inProgressSlices` is the list of open `size:multi-pr` slices currently landing PRs — that is the only definition of "in flight" the recap recognizes. If you find yourself reaching into a PRD's `## User Stories` to invent a closer, stop: those stories may have been cut, deferred, or never converted to real work. If `inProgressSlices` is empty, skip the closer entirely. Do not invent activity to fill the slot. The `upcoming` window is itself entirely forward-looking, so the closer concept does not apply to it.
...
- `openParents` — open `type:prd` and `type:epic` issues, the named bodies of work the team has committed to. Each carries a `kind` field (`"prd"` or `"epic"`), a `title`, and a `body` (which often contains a `## Solution` or `## User Stories` section that is the source-of-truth for stakeholder framing).
- `inProgressSlices` — standard `size:multi-pr` issues with the `in-progress` label. Each has a `parentNumber` (the PRD/epic it belongs to) and a `childProgress` block (`closed` and `total` PRs already shipped vs. queued under the slice).
- `readyQueue` — open `ready-for-agent` issues. Some are children of in-progress slices, some are direct children of PRDs, some are orphans. The label set distinguishes `size:1pr` from `size:multi-pr`.

Frame the prose around what is in motion right now and what is queued next, in stakeholder language:

- Lead with the most active body of work — typically the PRD or epic with the most in-progress slices and the most progress on its children. If two are equally active, lead with whichever has more user-perceptible weight.
...
- **Bold is permitted for feature or PRD names.** Use it sparingly and consistently — one or two bolded names at most.
...
```json
{
  ...
  "issuesClosed": [
    { "number": <n>, "title": "...", "labels": [...], "kind": "prd|epic|slice|leaf", "body": "...", "closedAt": "..." }
  ],
  "prdContext": [
...
```json
{
  ...
  "now": "<iso>",
  "openParents": [
    { "number": <n>, "title": "...", "labels": [...], "body": "...", "kind": "prd" | "epic" }
  ],
  "inProgressSlices": [
    { "number": <n>, "title": "...", "labels": [...], "body": "...", "parentNumber": <n>, "childProgress": { "closed": <n>, "total": <n> } }
  ],
...
## Step 4: Read the PRD context (past windows only)

For past windows (`today`, `week`), the helper's `prdContext` field contains, for every PRD or slice that closed in the window, the parent PRD's `## User Stories` section. This is a **translation aid**, not a feature checklist.

If a PRD or slice closed in the window, read its user stories *before* drafting the lead sentence. The user-meaning translation rule cannot be applied without this — without it you will fall back to paraphrasing PR titles, which is exactly the failure mode the rule exists to prevent.

**Bound how you use it.** A PRD's user stories capture original intent at PRD creation time. By the time the PRD ships, some stories have been cut (closed `wontfix`, dropped from scope, or rolled into a future PRD), and others have been deferred to a later body of work. Use the stories to find stakeholder-friendly language for features that *did* land, by cross-referencing each candidate story against `prsMerged` (and only `prsMerged`). If a story has no matching merged PR, it did not ship in this window — do not describe it as if it had.

If `prdContext` is empty (no PRDs or slices closed in the window), skip this step.
...
2. **Draft the lead sentence** in the chosen tier's voice. For headline tier, name the PRD/epic and give a one-sentence user-facing summary drawn from `prdContext`. For notable, anchor on the most user-visible change. For ordinary, just describe. For quiet, acknowledge honestly.
...
5. **Optionally add a forward-looking closer** if `inProgressSlices` contains a notable entry you can describe in plain English. The closer's source must be `inProgressSlices`, never a PRD's user stories. Skip the closer if `inProgressSlices` is empty or dominated by minor skill churn that wouldn't earn its own sentence in the body.
...
- **Closer source check.** When writing a past-window recap, every user-facing claim in the prose must trace back to an entry in `prsMerged`, and the optional "next up" closer must trace back to an entry in `inProgressSlices`. If you find yourself bridging from a PRD's `## User Stories` to invent prose without a merged-PR (for body) or in-progress-slice (for closer) backing, stop. That is the failure mode the rule exists to prevent.
- **Magnitude check (past windows).** On a day where a PRD/epic landed on `main`, the recap leads in headline tier. On a day with steady mid-volume commits and no closures, the recap reads as ordinary. On a day with 1 to 2 commits, the recap is a quiet acknowledgment.
