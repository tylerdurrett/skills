...
| 6 | Cleanup sweep | `/triage` + `/batch #<S>` over the deferred `cleanup` children, drain-until-dry (cap 3 rounds) | No open `needs-triage`+`cleanup` child of `<S>` remains |
...
`/batch`'s Settle phase auto-defers every non-blocking code-review finding on a merged task as a `needs-triage`+`cleanup` sub-issue of the slice — so the findings aren't lost when the task PR closes, but they *are* left as a pile for a human to triage and rubber-stamp later. Stage 6 closes that gap: it does the rubber-stamping itself, folding the cleanup into the same review surface.
...
- **Drain-until-dry, capped at 3 rounds.** A cleanup batch runs its own code review, which can defer *new* cleanup findings (cleanup-of-cleanup). So stage 6 loops: each round triages the open `needs-triage`+`cleanup` children and batches the now-ready ones; it stops when a round finds no such children left, or after 3 rounds — whichever comes first. The cap is a termination backstop, not an expected limit; convergence in 1–2 rounds is normal. If the cap is hit with cleanup still queued, that's reported, not an error.
