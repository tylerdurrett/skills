...
>
> - **BLOCKING** — exactly these three *plan-poisoning* categories (each corrupts more than the one child it names, so a batch built on it wastes a full parallel run): (1) slice scope drift — uncovered scope that changes the slice's boundary; (2) a sequencing reversal that breaks the dependency DAG; (3) hallucinated API surface that a task depends on. When genuinely unsure whether a finding is one of these three, classify as blocking.
