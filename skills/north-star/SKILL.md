...
`docs/north-star.md` is the project's vision/direction doc — the spine the planning layer hangs off. The roadmap below it (`docs/roadmap.md`) is the capacity-honest sequencing of bets; initiatives and features hang off the roadmap. The north star is what those layers are *for*.
...
gh issue list --label "size:initiative" --state closed --search "closed:>${LAST}" --json number,title,closedAt
gh pr list --state merged --base main --search "merged:>${LAST} head:feature/" --json number,title,mergedAt,headRefName
```
...
```
Since YYYY-MM-DD:
- Closed features: #<N> <title>, #<M> <title>
- Closed initiatives: #<N> <title>
- Feature-promotion PRs merged to main: #<#> <title> (head: feature/issue-<F>-<slug>)
```
...
2. **Run `/north-star`.** The skill parses the date, runs the three `gh` surveys (closed `size:feature`, closed `size:initiative`, merged `feature/issue-*` PRs since the parsed date), and surfaces a short list.
